<div>
    ...
    <x-ui.card class="max-w-[480px]">
        ...
        <div class="grid grid-cols-[1fr_2fr] gap-3 mb-4">
            <div>
                <label class="block text-[11.5px] text-ink_text-secondary mb-1">Points</label>
                <input type="number" wire:model="points" class="rj-input w-full">
                @error('points') <div class="text-danger text-[11px] mt-1">{{ $message }}</div> @enderror
            </div>
            <div>
                <label class="block text-[11.5px] text-ink_text-secondary mb-1">Reason</label>
                <input type="text" wire:model="reason" placeholder="e.g. goodwill, referral bonus" class="rj-input w-full">
                @error('reason') <div class="text-danger text-[11px] mt-1">{{ $message }}</div> @enderror
            </div>
        </div>

        <x-ui.button type="button" wire:click="award" variant="primary">Award Points</x-ui.button>
    </x-ui.card>
</div>
